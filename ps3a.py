...
def choose_word(hand, N=100):
    ...
    hand1 = hand_to_list(hand)
    
    for i in range(0, N):
        k = random.randint(0,length)
        seq = "".join(random.choices(hand1, k=k))
        seq
        if ps2.is_valid_word(seq, hand, word_list) == True: 
            tmp = ps2.get_word_score(seq, length)
            if tmp > bestscore:
                bestscore = tmp
                bestword = seq
...
def play_mc_hand(hand, N=100):
    ...
    while calculate_handlen(hand) > 0:
        best = choose_word(hand, N=100)
        if best[0] == None:
            break
        
        
        hand = ps2.update_hand(hand, best[0])
        wordls.append(best[0])
        handscore = handscore + best[1]
...
def play_n_mc_hand(hand, N, n):
    ...
    for i in range(0, n):
        temp1, temp2 = play_mc_hand(hand, N)
        #print(temp1)
        mc_hands.append(temp1)
        #print(mc_hands)
        mc_scores.append(temp2)
        #print(mc_scores)
        
        #hand = ps2.update_hand(hand, temp1)
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while handlen > 0:
        ...
        if word == '!!':
            ...
            sugword, sugscore = choose_word(hand, N=100) 
            print("Suggested word is", '"{}"'.format(sugword), "with score of", sugscore, "points.")
